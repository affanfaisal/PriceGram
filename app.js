...
const app = express();
__dirname = "D:DownloadsPriceGram";
...
app.get("/products", function (req, res) {
  if (req.isAuthenticated()) {
    ...
    con.query(
      "call get_product_by_category_vendor_title_page(" +
        category +
        ", " +
        vendor +
        ", " +
        ...
        ")",
      function (err, result, fields) {
        if (err) console.log("Search Product By All Error");
        products = result[0];
        if (q == '""') {
          q = "";
        } else {
        ...
        con.query(
          'SELECT IFNULL(category.name, "") as category_name, IFNULL(vendor.name, "") as vendor_name from category join vendor where category_id = IFNULL(' +
            category +
            ", category_id) and vendor_id = IFNULL(" +
            vendor +
            ", vendor_id)",
          function (err, result, fields) {
            if (err) console.log("Search Product VC Error");
            ...
            console.log(vendor_name, category_name);
...
app.get("/dashboard", function (req, res) {
  ...
  if (req.isAuthenticated()) {
    con.query(
      ...
      function (err, result, fields) {
        if (err) console.log("Dashboard Error");
        sale1 = result[0];
        con.query(
